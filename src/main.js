...
function createClientProfileFromHTML(formData) {
  try {
    ...
    // Try to sync to Wix (but don't fail if credentials missing)
    let wixSyncResult = null;
    let profileUrl = null;
    
    try {
      // Check if Wix is enabled
      const properties = PropertiesService.getScriptProperties();
      const skipWix = properties.getProperty('SKIP_WIX_SYNC');
      
      if (skipWix !== 'true') {
        // Attempt Wix sync
        wixSyncResult = createWixProfile(formData);
        profileUrl = generateWebAppProfileUrl(profileId);
        Logger.log('Wix sync successful');
      } else {
        Logger.log('Wix sync skipped - system in independent mode');
        profileUrl = `Profile created in Google Sheets only (Wix sync disabled)`;
      }
    } catch (wixError) {
      Logger.log('Wix sync failed (continuing without): ' + wixError.toString());
      profileUrl = `Profile created in Google Sheets only (Wix credentials not configured)`;
      wixSyncResult = { success: false, error: wixError.message };
    }
    ...
    return {
      ...
      sheetUrl: clientFolderUrl,
      wixSync: wixSyncResult,
      message: wixSyncResult?.success ? 'Profile created and synced to Wix!' : 'Profile created in Google Sheets (Wix sync unavailable)'
    };
